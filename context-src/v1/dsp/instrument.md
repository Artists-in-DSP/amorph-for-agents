...
5. **No C++:** `auto`, `unsigned`, `uint32_t`, `uint64_t`, `size_t`, `constexpr`, `static`, and local `Voice& v` aliases are invalid. Use `voices.at(i)` directly; `&` is legal only in function parameters; identifiers must be ASCII.
...
9. **Audio loop:** `void main()` must contain a continuous `loop { ... }`. Write `out <- value;` and then call exactly one `advance()` on every iteration. A one-shot body or a `for` loop that processes only one sample is invalid even if it compiles.
