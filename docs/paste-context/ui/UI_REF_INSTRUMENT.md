...
<!-- Source: InfinityLab commit 15cfd779 variant instrument -->
...
13. **Layout -- CRITICAL (plugin viewport, NOT a web page):**
    ...
    - [NO] NEVER fixed `px` width/height on `:host` / `.patch-root`
    - [NO] NEVER `attachShadow()` -- always use `this.innerHTML` (light DOM). Shadow DOM breaks body-level scroll suppression.
    - [OK] `:host, .patch-root { display: block; width: 100%; height: 100%; overflow: hidden; }` -- the ONLY correct form. `:host` alone is dead in the light DOM (no shadow root) -- always pair it with `.patch-root` and add that class to the element (`this.classList.add("patch-root")` next to `this.innerHTML = this.getHTML()`).
...
14. **Canvas sizing -- CRITICAL (prevents infinite layout-growth loop):**
    ...
    - [NO] NEVER `height: 100%` on a canvas that is a direct grid/flex item without `min-height: 0` on the item/track -- the measured height feeds back into the track and grows every frame
    - [OK] Take the canvas out of layout: wrapper `position: relative; min-height: 0; overflow: hidden` + canvas `position: absolute; inset: 0`; size the backing store once from the wrapper's `offsetWidth/offsetHeight` (or ResizeObserver)
...
class MyAlgoPatchView extends HTMLElement {
    constructor (patchConnection) {
        ...
        this.classList.add("patch-root"); // :host alone never matches in the light DOM
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :host, .patch-root { display: block; width: 100%; height: 100%; overflow: hidden;
...
3. **Control constructor MUST call `setValue(defaultValue, false)` at the end** -- calling only `updateVisuals()` updates the knob rotation but leaves the value label showing its initial HTML placeholder (`--`). The label only updates through `setValue`, not through `updateVisuals`. Same for any derived readout (Hz, Q, ms text, level numbers): paint it in the control constructor / setValue path, not only inside requestAnimationFrame -- rAF is throttled to zero while the plugin window is in the background, so loop-only paint reads as placeholders to run_ui_probe and to users returning to the window.
...
9. **Never `canvas.width = canvas.clientWidth` inside `requestAnimationFrame`** -- causes an infinite layout-growth loop and a scrolling UI. Size canvases ONCE in `connectedCallback` (e.g. `canvas.width = canvas.offsetWidth || 400`). Never `height: 100%` on a canvas that is a direct grid/flex item without `min-height: 0` -- prefer `position: absolute; inset: 0` inside a `position: relative; min-height: 0; overflow: hidden` wrapper.
