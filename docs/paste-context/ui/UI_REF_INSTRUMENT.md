...
<!-- Source: InfinityLab commit b5b1eb17 variant instrument -->
...
class ParameterControl {
    ...
    updateVisuals () {
        ...
        this.knob.style.setProperty("--arc", `${norm * 270}deg`);
    }
    onPointerDown (e) {
...
export default function createPatchView (patchConnection) {
    ...
    if (!window.customElements.get(name))
        window.customElements.define(name, MyAlgoPatchView);
    const view = document.createElement(name);
    view.pc = patchConnection;
    return view;
}
```
