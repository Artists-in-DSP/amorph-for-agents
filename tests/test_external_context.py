...
class ExternalContextTests(unittest.TestCase):
    ...
    def test_dsp_sources_cover_observed_generalised_failures(self):
        for path in sorted((ROOT / "context-src" / "v1" / "dsp").glob("*.md")):
            with self.subTest(path=path.name):
                ...
                self.assertIn("aliases are invalid", text)
                self.assertIn("`&` is legal only in function parameters", text)
                self.assertIn("directly", text)
                self.assertIn("input event float transportIn;", text)
                self.assertIn("reserved hidden host-fed endpoint", text)
                self.assertIn("never rename it to `paramN`", text)
                self.assertIn("add `[[ ... ]]`", text)
                self.assertIn('a "Transport In" UI control', text)
                self.assertIn("event transportIn(float value)", text)
                self.assertIn("never in `main()`", text)
                self.assertIn("int transportSlot=0;", text)
                self.assertIn("bool hostPlaying=false;", text)
                self.assertIn("float hostBpm=120.0f;", text)
                self.assertIn("int hostNumerator=4;", text)
                self.assertIn("int hostDenominator=4;", text)
                self.assertIn("float64 currentPpq=0.0;", text)
                self.assertIn("float64 hostBarStartPpq=0.0;", text)
                self.assertIn("if(transportSlot==0) hostPlaying=value>0.5f;", text)
                self.assertIn("transportSlot==1) hostBpm=clamp", text)
                self.assertIn("transportSlot==2) hostNumerator=max", text)
                self.assertIn("transportSlot==3) hostDenominator=max", text)
                self.assertIn("transportSlot==4) currentPpq=float64(value);", text)
                self.assertIn("else hostBarStartPpq=float64(value);", text)
                self.assertIn("transportSlot=(transportSlot+1)%6;", text)
    ...
    def test_dsp_foundations_are_version_pinned_and_semantically_complete(self):
        ...
        for required in (
            ...
            "zero occurrences of `processor.currentTime`",
            "never declare a local named `twoPi`",
            "Processor properties here are `frequency`, `period`, `id`, `session`",
            "`std::random::RNG rng;`",
            "`rng.getBipolar()`",
